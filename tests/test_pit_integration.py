...
class TestPitParser:
    ...
    async def test_parse_config_async_success(self, mocker):
        ...
        mock_parse = mocker.patch('services.pit_parser.store_productscraper.parse_config')
    ...
    async def test_parse_config_async_file_not_found(self, mocker):
        ...
        mocker.patch('services.pit_parser.store_productscraper.parse_config', side_effect=FileNotFoundError)
    ...
    async def test_fetch_page_async_selenium(self, mocker, mock_selenium):
        ...
        mock_fetch = mocker.patch('services.pit_parser.store_productscraper.fetch_page_selenium')
    ...
    async def test_fetch_page_async_requests(self, mocker):
        ...
        mock_fetch = mocker.patch('services.pit_parser.store_productscraper.fetch_page_requests')
    ...
    async def test_extract_product_data_async_success(self, mocker):
        ...
        mock_extract = mocker.patch('services.pit_parser.store_productscraper.extract_data_from_template')
        mock_extract.side_effect = ['Product Title', '99.99 €']
        mock_extract_package = mocker.patch('services.pit_parser.store_productscraper.extract_package_info')
        mock_extract_package.return_value = ('1.0 л', 1.0, 'л')
        mock_extract_price = mocker.patch('services.pit_parser.store_productscraper.extract_price_info')
        mock_extract_price.return_value = (99.99, 'EUR')
        mock_calculate = mocker.patch('services.pit_parser.store_productscraper.calculate_price_per_unit')
...
class TestPitDb:
    ...
    def test_add_price_history_new(self, sample_product):
        ...
        history = PriceHistory.select().where(PriceHistory.product == sample_product).first()
    ...
    def test_add_price_history_duplicate(self, sample_product):
        ...
        item = {'price': 100.0, 'unit_size': 1.0, 'unit_type': 'л', 'price_per_unit': 100.0}
        ...
        count = PriceHistory.select().where(PriceHistory.product == sample_product).count()
        assert count == 1
    ...
    def test_save_pit_results_error_handling(self, mocker, test_database):
        ...
        mocker.patch('services.pit_db.get_or_create_product', side_effect=Exception('DB error'))
