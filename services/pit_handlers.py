...
from services.pit_db import get_pit_products
...
async def pit_products_command(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not products:
            await message.reply("Товары PIT не найдены." + 
                               (f" Фильтр по магазину: '{store_filter}'" if store_filter else ""))
            return
        
        lines = []
        for p in products:
            unit_info = f" ({p.unit_size}{p.unit_type})" if p.unit_size and p.unit_type else ""
            price_per_unit = f", {p.price_per_unit:.2f} ₽/{p.unit_type}" if p.price_per_unit else ""
            lines.append(f"• {p.store} - {p.name}{unit_info}: {p.price:.2f} ₽{price_per_unit}")
...
async def price_per_unit_command(message: types.Message, state: FSMContext):
    ...
    try:
        product_id = int(args)
        product = Product.get_or_none(Product.id == product_id, Product.category == 'pit')
        if not product:
            await message.reply("Товар с указанным ID не найден или не является товаром PIT.")
            return
        
        # Получаем историю цен
        history = PriceHistory.select().where(
            PriceHistory.product == product
        ).order_by(PriceHistory.timestamp.desc()).limit(5)
        ...
        for h in history:
            lines.append(f"• {h.timestamp.strftime('%d.%m %H:%M')}: {h.price:.2f} ₽ "
...
async def compare_units_command(message: types.Message, state: FSMContext):
    ...
    args = message.get_args()
    if not args:
        await message.reply("Укажите название товара (частично): /compare_units <название>")
        return
    
    try:
        products = Product.select().where(
            (Product.category == 'pit') &
            (Product.name.contains(args))
        ...
        for p in products:
            unit_info = f" ({p.unit_size}{p.unit_type})" if p.unit_size and p.unit_type else ""
            lines.append(f"• {p.store}: {p.price:.2f} ₽{unit_info} → {p.price_per_unit or '?'} ₽/{p.unit_type or 'ед.'}")
